...
# output
tmp = c(seed = ii, K = K, Kexp = Kexp, b_ciscml = ciscML0_b, b_ciscml_dp = ciscMLDP0_b, se_ciscml = ciscML0_se, se_ciscml_dp = ciscMLDP0_se, cover_ciscml = ciscML0_b + 1.96* ciscML0_se > theta & ciscML0_b - 1.96* ciscML0_se < theta, cover_ciscml_dp = ciscMLDP0_b + 1.96* ciscMLDP0_se > theta & ciscMLDP0_b - 1.96* ciscMLDP0_se < theta, ciscml = ciscML0_b + 1.96* ciscML0_se < 0 | ciscML0_b - 1.96* ciscML0_se > 0, ciscml_dp = ciscMLDP0_b + 1.96* ciscMLDP0_se < 0 | ciscMLDP0_b - 1.96* ciscMLDP0_se > 0)
out = rbind(out, tmp)
} # end if
} # end loop
...
if (length(out)) {
colnames(out) = c('seed', 'K', 'Kexp', 'b_ciscml', 'b_ciscml_dp', 'se_ciscml', 'se_cml_cisdp', 'cover_ciscml', 'cover_ciscml_dp', 'ciscml', 'ciscml_dp')
